...
def validate_resolution(resolution: Union[int, List[int], Tuple[int, int]]) -> List[int]:
    """Validate and normalize resolution to [height, width] format.
    
    Args:
        resolution: Either an int (for square resolution) or a list/tuple of [h, w]
        
    Returns:
        List[int]: Resolution in [height, width] format
    """
    ...
    raise ValueError(f"Invalid resolution format: {resolution}. Expected int or [h, w] list/tuple")
